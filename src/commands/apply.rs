...
pub fn execute(palette_name: &str) -> Result<()> {
    formatter::header(&format!("Applying palette: {}", palette_name));
    ...
    for target in &config.targets {
        if let Err(e) = processor.process(target, &context, &palette) {
            formatter::error(&format!("Failed to process {}: {}", target.name, e));
        }
    }
    ...
    formatter::success("Theme applied successfully!");
    Ok(())
}
